...
const StudentCard1 = ({ students, headline }) => {
  ...
  const handleAccept = async (student_id) => {
    const response = await fetch(`http://localhost:3000/student/${student_id}`, {
      ...
      body: JSON.stringify({ hod_state: true }),
    });
        ...
        {students.map((student) => (
          ...
          >
            ...
            {student.hod_state === false ? (
              ...
              <div className="flex justify-center mt-4 space-x-4">
                <p className='text-gray-500 underline'>Forwarded to Principal</p>
              </div>
            )}
          </div>
        ))}
